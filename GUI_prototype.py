...
def load_spectral_index():
    ...
    try:
        ...
        messagebox.showinfo("Success", "Spectral Index loaded successfully!")
...
def select_and_display_image():
    ...
    try:
        if selected_folder is None:
            messagebox.showwarning("No Folder Selected", "Please select a folder containing images first.")
            return
...
def open_tsne_window():
    ...
    def tsne_2d():
        ...
        try:
            if flattened_images is None:
                messagebox.showwarning("No Data Loaded", "Please load the spectral index first.")
                return
    ...
    tsne_2d_button = tk.Button(top_level_window, text="t-SNE 2D", command=tsne_2d, font=("Arial", 12, "bold"))
    tsne_3d_button = tk.Button(top_level_window, text="t-SNE 3D", command=tsne_3d, font=("Arial", 12, "bold"))
    close_button = tk.Button(top_level_window, text="Close Window", command=close_tsne_window, font=("Arial", 12, "bold"))
...
def open_pca_window():
    ...
    # Create buttons in the additional window
    pca_2d_button = tk.Button(top_level_pca_window, text="PCA 2D", command=pca_2d, font=("Arial", 12, "bold"))
    pca_3d_button = tk.Button(top_level_pca_window, text="PCA 3D", command=pca_3d, font=("Arial", 12, "bold"))
    close_button = tk.Button(top_level_pca_window, text="Close Window", command=close_pca_window, font=("Arial", 12, "bold"))
...
root = tk.Tk()
root.title("Data Cube Visualization")  # Update the title of the Tkinter window
root.geometry("500x400")  # Set window size
#root.configure(bg="#e0f2f1")  # Set window background color

# Create the buttons and assign their respective functions
load_button = tk.Button(root, text="Load Spectral Index", command=load_spectral_index, font=("Arial", 12, "bold"))

select_image_button = tk.Button(root, text="Select Image", command=select_and_display_image, font=("Arial", 12, "bold"))

open_tsne_window_button = tk.Button(root, text="Open t-SNE Options", command=open_tsne_window, font=("Arial", 12, "bold"))

open_pca_window_button = tk.Button(root, text="Open PCA Options", command=open_pca_window, font=("Arial", 12, "bold"))

# Place the buttons in the main window
load_button.pack(pady=20)
select_image_button.pack(pady=20)
open_tsne_window_button.pack(pady=20)
open_pca_window_button.pack(pady=20)
