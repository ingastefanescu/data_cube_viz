# -*- coding: utf-8 -*-
"""
Created on Fri Jun 14 18:48:09 2024
...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 14 18:45:10 2024
...
def plot_pixel_values(x, y):
    global selected_folder, clicked_coordinates, image_dates

    try:
        if clicked_coordinates is None:
            raise ValueError("No coordinates have been clicked.")

        # Initialize lists to store pixel values and dates
        pixel_values = []
        #image_dates = []
        ...
        for file_path in image_files:
            ...
            # Assuming image_dates is defined somewhere (not in the provided code snippet)
            # Replace this with the actual logic to fetch dates from your data
            #date_value = get_image_date(file_path)  # Example function to get date from file_path
            #image_dates.append(date_value)

        # Plot pixel values against image_dates
        plt.figure()
        plt.plot(image_dates, pixel_values, marker='o', linestyle='-', color='b')
        plt.xlabel("Image Dates")
        plt.ylabel("Pixel Value")
        plt.title(f"Pixel Value at ({x}, {y}) Across Images")
        plt.show()
...
def get_image_date(file_path):
    # Replace this function with your logic to extract date from file_path
    # Example: return datetime.datetime.strptime(os.path.basename(file_path), "%Y%m%d").date()
    return os.path.basename(file_path)  # Just returning basename as example
