...
def plot_pixel_values(x, y):
    global selected_folder, clicked_coordinates
    ...
    try:
        ...
        print(image_indices)
        # Plot normalized pixel values against image_indices using Seaborn lineplot
        plt.figure()
        sns.lineplot(x=np.arange(1, len(image_indices) + 1), y=normalized_values, marker='o', color='b', palette='viridis',
                  legend='full', lw=3)
        plt.xlabel("Image Index")
        plt.ylabel("Normalized Pixel Value")
        plt.title(f"Normalized Pixel Value at ({x}, {y}) Across Images")
        plt.grid(True)
        plt.show()
