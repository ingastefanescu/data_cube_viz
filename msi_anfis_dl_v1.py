# -*- coding: utf-8 -*-
"""
Created on Wed Feb  7 19:29:18 2024
...
input_var = ctrl.Antecedent(np.arange(*input_universe, step=0.1), 'input')
output_var = ctrl.Consequent(np.arange(*output_universe, step=0.1), 'output')

# Generate fuzzy membership functions for the input and output
input_var['low'] = fuzz.trimf(input_var.universe, [input_universe[0], np.percentile(input_universe, 25), input_universe[1]])
input_var['medium'] = fuzz.trimf(input_var.universe, [np.percentile(input_universe, 25), np.percentile(input_universe, 75), np.percentile(input_universe, 75)])
input_var['high'] = fuzz.trimf(input_var.universe, [np.percentile(input_universe, 75), input_universe[1], input_universe[1]])

output_var['low'] = fuzz.trimf(output_var.universe, [output_universe[0], np.percentile(output_universe, 25), output_universe[1]])
output_var['medium'] = fuzz.trimf(output_var.universe, [np.percentile(output_universe, 25), np.percentile(output_universe, 75), np.percentile(output_universe, 75)])
output_var['high'] = fuzz.trimf(output_var.universe, [np.percentile(output_universe, 75), output_universe[1], output_universe[1]])
...
# Create a simple neural network using scikit-learn MLPRegressor
num_inputs = len(input_var.terms)
num_outputs = len(output_var.terms)
num_hidden = 5  # You can adjust the number of hidden neurons

neural_network = MLPRegressor(hidden_layer_sizes=(num_hidden,), activation='relu', max_iter=1000)

# Connect the neural network to the ANFIS system
anfis_model.model = neural_network
