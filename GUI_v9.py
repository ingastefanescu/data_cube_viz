...
def plot_pixel_values(x, y):
    ...
    try:
        ...
        for idx, file_path in enumerate(image_files, start=1):
            ...
            # Append index (1 to len(image_files)) as image indices
        ...
        # Plot normalized pixel values against image_indices using Plotly scatter plot
        fig = go.Figure()
        # fig.add_trace(go.Scatter(x=np.arange(1, len(image_indices) + 1), y=normalized_values.ravel(),
        #                          mode='lines+markers', name=f"Pixel at ({x}, {y})"))
        fig.add_trace(go.Scatter(x=image_dates, y=normalized_values.ravel(),
                                 mode='lines+markers', name=f"Pixel at ({x}, {y})"))
        fig.update_layout(title=f"Normalized Pixel Value at ({x}, {y}) Across Images",
                          xaxis_title="Image Index", yaxis_title="Normalized Pixel Value",
